...
void main() {
  group('TimeSpan end time is on the next day', () {
    test(
        '.inMinutes is 1440 minutes when start time 0:00 and end time 0:00 of the next day',
        () {
      var span = TimeSpan('00:00', '00:00');
      expect(span.inMinutes, equals(span.minutesPerDay));
    }, skip: 'not implemented yet');
    test(
        '.inHours is 24 hours when start time 0:00 and end time 0:00 of the next day',
        () {
      var span = TimeSpan('00:00', '00:00');
  ...
  group('TimeSpan end time is on the same day', () {
    ...
    test(
        '.inMinutes is 1439 minutes when start time 0:00 and end time 23.59 on the same day',
        () {
      var span = TimeSpan('00:00', '23:59');
      expect(span.inMinutes, equals(span.minutesPerDay - 1));
    });
    test(
        '.inHOurs is 23.98 hours when start time 0:00 and end time 23.59 on the same day',
        () {
      var span = TimeSpan('00:00', '23:59');
      expect(span.inHours, equals(23.98));
    });
